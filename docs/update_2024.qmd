...
The 2024 update of the WFSVI layer differs from previous years in that it is not clipped to census block groups (CBG) that include WUI (like it was in previous years).
...
The WFSVI is the layer with all Census Block Groups in Colorado and should be used for the FRWRM grant.
...
#print map
mapview::mapshot(map,url = "qualify_or_not_wui.html")
...
For reference, the interactive map accessible [here](qualify_or_not_wui.html) is the layer displayed above but clipped to the WUI layer.
