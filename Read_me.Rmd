...
delist<-function(geog){
  ...
  dplyr::left_join(.,dplyr::select(SVI,census_block_group))
}
SVI_layer<-SVI%>%
  left_join(.,CO,by=c("census_block_group"="GEOID"))%>%
  st_as_sf()%>%
  st_make_valid()
rm(SVI,st_out,SVI_comp)
pal<-colorQuantile("Reds",na.omit(SVI_layer$wfsvi))
...
SVI_pop<-SVI_layer%>%
  dplyr::select(wfsvi,variables$variable)%>%
  mutate(across(is.numeric,round,3))%>%
  mutate(across(is.numeric,~100*.x))%>%
  mutate(across(is.numeric & !median_hh_income_3 & !gini_education & !gini_income & !wfsvi,paste,"%"),.keep="all")%>%
  mutate(across(wfsvi,str_replace,'^0$',"Non-WUI"),.keep="unused")%>%
  mutate(across(median_hh_income_3,~.x/100),.keep="unused")%>%
  mutate(across(median_hh_income_3,paste,'USD'),.keep="unused")%>%
  rename('SVI Rank'=wfsvi,
