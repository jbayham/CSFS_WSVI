...
SVI <- SVI_dat%>%
  mutate(overall_sum_SVI = ((weights[1]*poverty_percent_below_1)+(weights[2]*civ_labor_force_unemployed_percent_2)+
                              weights[3]*-1*median_hh_income_3)+(weights[4]*no_hs_degree_percent_4)+(weights[5]*over_65_percent_5)+
           (weights[6]*under_18_percent_6)+(weights[7]*disabled_adult_percent_7)+(weights[8]*single_householder_percent_8)+
           (weights[9]*percent_minority_9)+(weights[10]*eng_proficiency)+(weights[11]*over_10)+(weights[12]*mobile_units)+
           (weights[13]*over_1_person_room_percent_13)+(weights[14]*no_vehicle_percent_14)+
           (weights[15]*group_quarters_percent_15)+(weights[16]*gini_income)+(weights[17]*gini_education))%>%
